...
//    }
//
//    public void softDeleteProduct(Long id) throws ProductException {
...
//
//    @Override
//    public void deleteProductPermanently(Long id) throws ProductException {
//        Product product = getProductById(id);
//        if (product == null) {
//            throw new ProductException("Product not found with id: " + id);
//        }
...
public class ProductServiceImplementation implements ProductService {
    ...
    /**
     * ✅ Create new product and auto-create category hierarchy if not exists
     */
    @Override
    public Product createProduct(CreateProductRequest req) {
        ...
        product.setCreatedAt(LocalDateTime.now());
        product.setQuantity(req.getQuantity());
        product.setActive(true); // ✅ newly created product is active

        return productRepository.save(product);
    }

    /**
     * ✅ Get all active products with filters (pagination, colors, sizes, etc.)
     */
    @Override
    public Page<Product> getAllProduct(String category, List<String> colors,
                                       List<String> sizes, Integer minPrice, Integer maxPrice, Integer minDiscount, String sort,
        ...
        // fetch only active products from DB
        List<Product> products = productRepository.filterProducts(category, minPrice, maxPrice, minDiscount, sort)
                .stream()
                .filter(Product::isActive) // ✅ only active
                .collect(Collectors.toList());
    ...
    public Product getProductById(Long id) throws ProductException {
        ...
        if (opt.isPresent() && opt.get().isActive()) {
            ...
            throw new ProductException("Product not found or inactive with id " + id);
        }
    }
    ...
    /**
     * ✅ Permanent Delete — remove completely from database
     */
    @Override
    public void deleteProductPermanently(Long id) throws ProductException {
    ...
    /**
     * ✅ Fetch available filter options (brands, colors, sizes, stock)
     */
    @Override
    public Map<String, List<String>> getFilterOptions(String category) {
        ...
        List<Product> products = productRepository.findAll()
                .stream()
                .filter(Product::isActive) // ✅ Only active products
                .filter(p -> p.getCategory().getName().equalsIgnoreCase(category))
                .collect(Collectors.toList());
    ...
    public Product makeProductActive(Long productId) throws ProductException {
        ...
        if (Boolean.TRUE.equals(product.getActive())) {
            throw new ProductException("Product is already active");
        }
